...
public class BookIssueServlet extends HttpServlet {
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        IssuedBook issuedBook = new IssuedBook(new BookEdition(editionId), new MemberShip(memberId),
                DateUtil.getCurrentDate(), returnDate);

        MemberShip memberShip = new MemberShip();
        memberShip.setMemberId(memberId);
        boolean flag = memberShip.checkStatus();
        if (flag) {
            flag = issuedBook.issueBook();
            if (flag) {
